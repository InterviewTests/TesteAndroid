...
public class BrPhoneNumberFormatter implements TextWatcher {
    ...
    private final int MAX_LENGHT = 11;

    private boolean formatting; // this is a flag wich prevents the stack(onTextChanged)
    ...
    private String formatUsNumber(Editable text) {
        ...
        if (totalDigitCount > MAX_LENGHT) {
            allDigitString = allDigitString.substring(0, MAX_LENGHT);
            totalDigitCount--;
        }

        // Check if we are dealing with the new phone format, with an additional digit
        boolean isLonger = totalDigitCount == MAX_LENGHT;
